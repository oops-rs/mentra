...
use crate::runtime::{RuntimeError, store::TaskStore};
...
pub(crate) fn execute_with_store<S: TaskStore + ?Sized>(
    ...
    match tool {
        TaskIntrinsicTool::Create => {
            ...
            mutate_store_tasks(store, namespace, move |tasks| {
                ...
                for blocker_id in &parsed.blocked_by {
                    graph::add_dependency(tasks, *blocker_id, task_id)
        ...
        TaskIntrinsicTool::Claim => {
            ...
            mutate_store_tasks(store, namespace, move |tasks| {
                let claimed = match parsed.task_id {
                    Some(task_id) => {
                        let task = store::find_task_mut(tasks, task_id)
                            .map_err(|error| error.to_string())?;
                        store::validate_claimable(task, &owner)
                            .map_err(|error| error.to_string())?;
        ...
        TaskIntrinsicTool::Update => {
            ...
            mutate_store_tasks(store, namespace, move |tasks| {
                ...
                {
                    let task =
                        store::find_task_mut(tasks, task_id).map_err(|error| error.to_string())?;
                ...
                for blocker_id in &parsed.add_blocked_by {
                    graph::add_dependency(tasks, *blocker_id, task_id)
                        .map_err(|error| error.to_string())?;
                }
                for blocker_id in &parsed.remove_blocked_by {
                    graph::remove_dependency(tasks, *blocker_id, task_id)
                        .map_err(|error| error.to_string())?;
                }
                for dependent_id in &parsed.add_blocks {
                    graph::add_dependency(tasks, task_id, *dependent_id)
                        .map_err(|error| error.to_string())?;
                }
                for dependent_id in &parsed.remove_blocks {
                    graph::remove_dependency(tasks, task_id, *dependent_id)
                        .map_err(|error| error.to_string())?;
                }
...
fn mutate_store_tasks(
    ...
    let result = {
        let mut mutation = |tasks: &mut Vec<TaskItem>| match operation(tasks) {
            Ok(value) => {
                output = Some(value);
                Ok(())
            }
            Err(error) => {
                ...
                Err(RuntimeError::InvalidTask(error))
            }
        };
        store.mutate(namespace, &mut mutation)
    };

    match result {
        Ok(()) => output.ok_or_else(|| "Task mutation did not produce a result".to_string()),
        Err(_) if task_error.is_some() => Err(task_error.expect("checked above")),
